...
def print_stats(model_config, x, y, model):
    ...
    d = print_forecast(model_config["model_name"], model, beginning_day=len(x), limit=model_config["days_to_predict"])
...
def model_handler(model_config):
    ...
    return print_stats(model_config, x, y, model)
...
class predict(Resource):
    ...
    def get(self):    
        ...
        z = model_handler(model_config)
        return jsonify({"prediction":z.tolist()})
